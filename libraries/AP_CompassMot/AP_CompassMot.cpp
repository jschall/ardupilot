...
void AP_CompassMot::update_gyro(Vector3f gyro) {
    ...
    Vector3f omega;
    if(_gyro_buffer[0].size() >= delay_elements && delay_elements != 0.0f) {
        //interpolate
        float interp = delay_elements-(int8_t)delay_elements;
        omega.x = _gyro_buffer[0].pop_front()*interp;
        omega.x += _gyro_buffer[0].front()*(1.0f-interp);
        omega.y = _gyro_buffer[1].pop_front()*interp;
        omega.y += _gyro_buffer[1].front()*(1.0f-interp);
        omega.z = _gyro_buffer[2].pop_front()*interp;
        omega.z += _gyro_buffer[2].front()*(1.0f-interp);
    } else {
        omega = gyro;
    }

    _gyro_buffer[0].push_back(gyro.x);
    _gyro_buffer[1].push_back(gyro.y);
    _gyro_buffer[2].push_back(gyro.z);
    _rottotal += omega.length() * _gyro_deltat;
    _rot_matrix.rotate(omega * _gyro_deltat);
    _rot_matrix.normalize_rot();
}

bool AP_CompassMot::update_compass(Vector3f mag) {
    if(_gyro_buffer[0].is_empty()) {
