...
TEST(COBSTest, StreamingDecoderErrorRecovery)
{
    ...
    EXPECT_FALSE(frame_complete);
    EXPECT_EQ(decoder.get_state(), AP_Networking_COBS::Decoder::State::ERROR);
...
// Test streaming decoder - zero byte in non-zero section (error)
...
TEST(COBSTest, FuzzTestEthernetFrames)
{
    ...
    for (uint32_t iter = 0; iter < num_iterations; iter++) {
        ...
        for (size_t i = 0; i < encoded_len; i++) {
            ...
            if (decoder.get_state() == AP_Networking_COBS::Decoder::State::ERROR) {
                break;
            }
        }
        
        // If not complete and no error, try end-of-input
        if (!frame_complete && decoder.get_state() != AP_Networking_COBS::Decoder::State::ERROR) {
            frame_complete = decoder.process_end_of_input();
        }
    ...
    for (size_t size_idx = 0; size_idx < sizeof(edge_case_sizes)/sizeof(edge_case_sizes[0]); size_idx++) {
        ...
        if (frame_size <= streaming_max_frame_size) {
            ...
            for (size_t i = 0; i < encoded_len; i++) {
                ...
                ASSERT_NE(decoder.get_state(), AP_Networking_COBS::Decoder::State::ERROR) 
