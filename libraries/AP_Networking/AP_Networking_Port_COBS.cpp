...
void AP_Networking_Port_COBS::process_rx()
{
    ...
    while (uart->available() > 0) {
        ...
        if (cobs_decoder.process_byte(byte)) {
            ...
            cobs_decoder.reset();
            // Validate CRC (little-endian at end)
            if (frame_len < 4) {
                rx_errors++;
                continue;
            }
            ...
            if (rx_crc != calc_crc) {
                crc_errors++;
                // Store debug info every 10th CRC error for AP_Periph to print
                if ((crc_errors % 10) == 0) {
                    ...
                    // Store last 8 bytes (should contain CRC)
            ...
            // Track maximum successfully decoded frame size
            if (frame_len > max_successful_frame_len) {
                max_successful_frame_len = frame_len;
            }
            // Check for keepalive marker; otherwise route any valid-sized frame
            ...
            // else: keepalive or invalid size; ignore silently
