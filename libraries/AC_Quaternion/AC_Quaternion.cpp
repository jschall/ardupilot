...
void AC_Quaternion::rotation_matrix(Matrix3f &m) const
{
    float q3q3 = q3 * q3;
    float q3q4 = q3 * q4;
    float q2q2 = q2 * q2;
...
void AC_Quaternion::from_rotation_matrix(const Matrix3f &m)
{
...
void AC_Quaternion::earth_to_body(Vector3f &v) const
{
    Matrix3f m;
    rotation_matrix(m);
    v = m * v;
...
void AC_Quaternion::from_euler(float roll, float pitch, float yaw)
{
...
void AC_Quaternion::from_axis_angle(Vector3f v) {
...
void AC_Quaternion::from_axis_angle(Vector3f axis, float theta) {
    if(theta == 0.0f) {
        q1 = 1.0f;
        q2=q3=q4=0.0f;
    }
...
void AC_Quaternion::rotate(Vector3f v) {
    AC_Quaternion r;
    r.from_axis_angle(v);
    (*this) *= r;
}

void AC_Quaternion::to_axis_angle(Vector3f &v) {
...
void AC_Quaternion::from_axis_angle_fast(Vector3f v) {
    float theta = v.length();
    if(theta == 0.0f) {
        q1 = 1.0f;
        q2=q3=q4=0.0f;
...
void AC_Quaternion::from_axis_angle_fast(Vector3f axis, float theta) {
    float t2 = theta/2.0f;
    float sqt2 = sq(t2);
...
void AC_Quaternion::rotate_fast(const Vector3f &v) {
    float theta = v.length();
...
void AC_Quaternion::to_euler(float &roll, float &pitch, float &yaw) const
{
...
float AC_Quaternion::length(void) const
{
    return sqrtf(sq(q1) + sq(q2) + sq(q3) + sq(q4));
}

AC_Quaternion AC_Quaternion::inverse(void) const
{
    return AC_Quaternion(q1, -q2, -q3, -q4);
}

void AC_Quaternion::normalize(void)
{
...
AC_Quaternion AC_Quaternion::operator*(AC_Quaternion v) {
    AC_Quaternion ret;
    float &w1 = q1;
    float &x1 = q2;
    float &y1 = q3;
