...
static void rtl_climb_start()
{
    ...
    destination.z = inertial_nav.alt_above_home_cm_to_alt_above_origin_cm(rally_point.alt);
#else
    destination.z = inertial_nav.alt_above_home_cm_to_alt_above_origin_cm(get_RTL_alt());
#endif
...
static void rtl_descent_run()
{
    ...
    pos_control.set_alt_target_with_slew(inertial_nav.alt_above_home_cm_to_alt_above_origin_cm(g.rtl_alt_final), G_Dt);
    pos_control.update_z_controller();
    ...
    rtl_state_complete = fabs(g.rtl_alt_final - inertial_nav.get_alt_above_origin_cm()) < 20.0f;
}
