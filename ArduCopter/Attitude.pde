...
get_rate_roll(int32_t target_rate)
{
    ...
    // prevent i from getting larger positive or negative if we've hit our limit
    /*i = g.pid_rate_roll.get_integrator();
    if(!(motors.reached_limit(AP_MOTOR_ROLLPITCH_LIMIT) && ((rate_error>0 && i>0) || (rate_error<0 && i<0)) )) {
        i = g.pid_rate_roll.get_i(rate_error, G_Dt);
    }*/
    i = g.pid_rate_roll.get_i(rate_error, G_Dt);
