...
*/
void StallDetection::update(void)
{
    ...
    if ((is_stalled_initial != is_stalled()) && is_stalled()) {
        ...
        confidence = 1; // jump this up to 1.0 (100%) to give us some hysterisis
        
#if STALL_RECOVERY_ENABLED
        // Trigger stall recovery
        if (plane.mode_stallrecovery.auto_recovery_enabled) {
            plane.set_mode(plane.mode_stallrecovery, ModeReason::STALL_RECOVERY);
        }
#endif
    }
    log();
...
bool StallDetection::detect()
{
    ...
    is_stalled &= single_check(DetectMethod::BAD_DESCENT, uncommanded_altitude_loss);


    // we're sinking faster than a controlled sink is allowed to be
    const float sink_rate = plane.TECS_controller.get_max_sinkrate();
    is_stalled &= single_check(DetectMethod::SINKRATE_2X_MAX, plane.auto_state.sink_rate > (sink_rate * 2));
    is_stalled &= single_check(DetectMethod::SINKRATE_4X_MAX, plane.auto_state.sink_rate > (sink_rate * 4));
    ...
    is_stalled &= single_check(DetectMethod::BAD_ROLL_20DEG, roll_error_cd > 2000);
    is_stalled &= single_check(DetectMethod::BAD_ROLL_30DEG, roll_error_cd > 3000);
    ...
    is_stalled &= single_check(DetectMethod::BAD_PITCH_10DEG, pitch_error_cd > 1000);
    is_stalled &= single_check(DetectMethod::BAD_PITCH_20DEG, pitch_error_cd > 2000);
    is_stalled &= single_check(DetectMethod::BAD_PITCH_30DEG, pitch_error_cd > 3000);
    is_stalled &= single_check(DetectMethod::BAD_PITCH_40DEG, pitch_error_cd > 4000);
    ...
    is_stalled &= single_check(DetectMethod::BAD_ALT_10m, altitude_error > 10.0f);
    is_stalled &= single_check(DetectMethod::BAD_ALT_20m, altitude_error > 20.0f);
    is_stalled &= single_check(DetectMethod::BAD_ALT_40m, altitude_error > 40.0f);
    is_stalled &= single_check(DetectMethod::BAD_ALT_60m, altitude_error > 60.0f);

    return is_stalled;
}

bool StallDetection::single_check(const DetectMethod _bitmask, const bool check)
{
    ...
    return (plane.g2.stall_detection_bitmask & bitmask) != 0;
}
